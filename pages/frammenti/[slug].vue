      ...
      <ContentDoc>
        <template #default="{ doc }">
          ...
          <div class="flex items-center gap-x-2 text-xs">
            ...
            <span v-else class="sm:hidden rounded-full bg-violet-300 py-1.5 px-3 font-medium text-white select-none hover:shadow-lg">
              {{ doc.book.substring(0,23) + "..." }}
            </span>
            <span class="rounded-full bg-rose-300 py-1.5 px-3 font-medium text-white select-none hover:shadow-lg">
              {{ doc.readingTime }}
            </span>
          </div>
